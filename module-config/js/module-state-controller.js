...
async function dispatch(action) {
  switch (action.type) {
    case ACTIONS.LOAD_MODULE:
      // console.log(`%c[Controller] ACTION: LOAD_MODULE triggered for moduleId: ${action.payload.moduleId}`, 'color: blue; font-weight: bold;');
      DMState.setUIState(UI_STATES.LOADING);
      try {
        const response = await ajax.getModule(action.payload.moduleId);
        // console.log('[Controller] AJAX getModuleData response:', response);
        if (response.success) {
          ...
          // console.log('[Controller] Mapped state to set:', mappedState);
      ...
      // console.log(`%c[Controller] ACTION: LOAD_MODULE finished for moduleId: ${action.payload.moduleId}`, 'color: blue; font-weight: bold;');
      break;
...
export { dispatch, subscribe, ACTIONS }; 