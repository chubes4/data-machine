...
// The required variables ($db_projects, $db_modules) should now be passed 
// directly from the calling Data_Machine_Admin_Page::display_admin_page method.

// Check if required variables are set (basic safety check)
if (!isset($db_projects) || !$db_projects instanceof Data_Machine_Database_Projects) {
    echo '<div class="error"><p>Error: Required project database service not available in template.</p></div>';
    return;
}
if (!isset($db_modules) || !$db_modules instanceof Data_Machine_Database_Modules) {
    echo '<div class="error"><p>Error: Required module database service not available in template.</p></div>';
    return;
}
...
// $db_projects is now passed directly
...
// $db_modules is now passed directly and already instantiated correctly
...
<div class="wrap">
    ...
    <div class="current-module-display" style="margin-bottom: 20px;">
        ...
        <?php if ($current_module): ?>
            ...
            <p><small>Configure modules in <a href="<?php echo esc_url(admin_url('admin.php?page=dm-module-config')); ?>">Settings</a></small></p>
        <?php else: ?>
            <div class="notice notice-error">
                <p>No module selected - please <a href="<?php echo esc_url(admin_url('admin.php?page=dm-module-config')); ?>">configure a module in Settings</a></p>
            </div>
        <?php endif; ?>
    </div>
