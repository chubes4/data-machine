...
class Data_Machine_OAuth_Facebook {
     ...
     * Constructor.
     *
     * @param string $client_id     Facebook App ID.
     * @param string $client_secret Facebook App Secret.
     * @param Data_Machine_Logger|null $logger Optional Logger instance.
     */
    public function __construct(string $client_id, string $client_secret, ?Data_Machine_Logger $logger = null) {
        $this->client_id = $client_id;
        $this->client_secret = $client_secret;
        // Define the redirect URI - must match one configured in the Facebook App settings
        $this->redirect_uri = admin_url('admin.php?page=dm-api-keys&dm_oauth_callback=facebook'); // Example callback URL
        $this->logger = $logger;
    }
    ...
    public function handle_callback(int $user_id, string $code, string $state): bool|WP_Error {
        $this->logger?->info('Handling Facebook OAuth callback.', ['user_id' => $user_id]);
        ...
        if (empty($stored_state) || !hash_equals($stored_state, $state)) {
            $this->logger?->error('Facebook OAuth Error: State mismatch.', ['user_id' => $user_id]);
        ...
        if (is_wp_error($response)) {
            $this->logger?->error('Facebook OAuth Error: Token request failed.', ['user_id' => $user_id, 'error' => $response->get_error_message()]);
        ...
        if ($http_code !== 200 || empty($data['access_token'])) {
            ...
            $this->logger?->error('Facebook OAuth Error: Token exchange failed.', ['user_id' => $user_id, 'http_code' => $http_code, 'response' => $body]);
        ...
        if (is_wp_error($long_lived_token_data)) {
            $this->logger?->error('Facebook OAuth Error: Failed to exchange for long-lived token.', [
                'user_id' => $user_id,
        ...
        if (is_wp_error($page_credentials)) {
            $this->logger?->error('Facebook OAuth Error: Failed to fetch page credentials.', [
                'user_id' => $user_id,
        ...
        // Select the first page found. TODO: Implement page selection UI if multiple pages exist.
        ...
        $encrypted_page_token = Data_Machine_Encryption_Helper::encrypt($page_access_token);
        if ($encrypted_page_token === false) {
             $this->logger?->error('Facebook OAuth Error: Failed to encrypt page access token.', ['user_id' => $user_id]);
        ...
        if (!is_wp_error($profile_info)) {
            ...
            $user_profile_name = $profile_info['name'] ?? 'ErrorFetchingName';
             ...
             $this->logger?->warning('Facebook OAuth Warning: Failed to fetch user profile info.', [
                 'user_id' => $user_id,
        ...
        $encrypted_user_token = Data_Machine_Encryption_Helper::encrypt($access_token);
        if ($encrypted_user_token === false) {
             $this->logger?->error('Facebook OAuth Error: Failed to encrypt user access token.', ['user_id' => $user_id]);
    ...
    public static function get_access_token(int $user_id): ?string {
        ...
        if (isset($account['token_expires_at']) && time() > $account['token_expires_at']) {
             // TODO: Implement a mechanism to notify the user or attempt re-auth/refresh later.
             return null; // Return null if expired
        }

        // Decrypt token
        $decrypted_token = Data_Machine_Encryption_Helper::decrypt($account['user_access_token']);
    ...
    public static function get_page_access_token(int $user_id): ?string {
        ...
        // Decrypt page token
        $decrypted_token = Data_Machine_Encryption_Helper::decrypt($account['page_access_token']);
        if($decrypted_token === false) {
            return null; // Return null if decryption fails
        }
    ...
    private function exchange_for_long_lived_token(string $short_lived_token): array|WP_Error {
        $this->logger?->info('Exchanging Facebook short-lived token for long-lived token.');
        $params = [
        ...
        if (is_wp_error($response)) {
            $this->logger?->error('Facebook OAuth Error: Long-lived token request failed.', ['error' => $response->get_error_message()]);
        ...
        if ($http_code !== 200 || empty($data['access_token'])) {
            $error_message = $data['error']['message'] ?? $data['error_description'] ?? 'Failed to retrieve long-lived access token from Facebook.';
             $this->logger?->error('Facebook OAuth Error: Long-lived token exchange failed.', ['http_code' => $http_code, 'response' => $body]);
        ...
        $this->logger?->info('Successfully obtained Facebook long-lived token.');
    ...
    private function get_page_credentials(string $user_access_token, int $user_id): array|WP_Error {
        $this->logger?->info('Fetching Facebook page credentials.', ['user_id' => $user_id]);
        ...
        if (is_wp_error($response)) {
            $this->logger?->error('Facebook Page Fetch Error: Request failed.', ['user_id' => $user_id, 'error' => $response->get_error_message()]);
        ...
        if ($http_code !== 200 || !isset($data['data'])) {
            ...
            $this->logger?->error('Facebook Page Fetch Error: API error.', ['user_id' => $user_id, 'http_code' => $http_code, 'response' => $body]);
            return new WP_Error('facebook_page_api_error', $error_message, $data);
        }

        if (empty($data['data'])) {
            $this->logger?->error('Facebook Page Fetch Error: No pages found for this user.', ['user_id' => $user_id, 'response' => $body]);
        ...
        // TODO: Implement selection logic if multiple pages are returned.
        $first_page = $data['data'][0];

        if (empty($first_page['id']) || empty($first_page['access_token']) || empty($first_page['name'])) {
             $this->logger?->error('Facebook Page Fetch Error: Incomplete data for the first page.', ['user_id' => $user_id, 'page_data' => $first_page]);
        ...
        $this->logger?->info('Successfully fetched credentials for Facebook page.', ['user_id' => $user_id, 'page_id' => $first_page['id']]);
    ...
    public function handle_oauth_callback_check() {
        ...
        if (isset($_GET['error'])) {
            ...
            $this->logger?->warning('Facebook OAuth Error in callback:', ['error' => $error, 'description' => $error_description]);
        ...
        if (!isset($_GET['code']) || !isset($_GET['state'])) {
            $this->logger?->error('Facebook OAuth Error: Missing code or state in callback.', ['query_params' => $_GET]);
        ...
        if (!current_user_can('manage_options')) {
             $this->logger?->error('Facebook OAuth Error: User does not have permission.', ['user_id' => get_current_user_id()]);
        ...
        // Retrieve stored app credentials for this user
        ...
        if (empty($facebook_account) || empty($facebook_account['app_id']) || empty($facebook_account['app_secret'])) {
            $this->logger?->error('Facebook OAuth Error: App credentials not found for user.', ['user_id' => $user_id]);
            wp_redirect(add_query_arg('auth_error', 'config_missing', admin_url('admin.php?page=dm-api-keys')));
            exit;
        }

        // Re-instantiate self with user's credentials to handle the callback
        $oauth_handler = new self($facebook_account['app_id'], $facebook_account['app_secret'], $this->logger);
        $result = $oauth_handler->handle_callback($user_id, $code, $state);

        if (is_wp_error($result)) {
            $error_code = $result->get_error_code();
            $error_message = $result->get_error_message();
            $this->logger?->error('Facebook OAuth Callback Failed.', ['user_id' => $user_id, 'error_code' => $error_code, 'error_message' => $error_message]);
            ...
            $this->logger?->info('Facebook OAuth Callback Successful.', ['user_id' => $user_id]);
