...
class Facebook {
    ...
    public function handle_oauth_callback_check() {
        ...
        if (empty($app_id) || empty($app_secret)) {
            $this->logger?->error('Facebook OAuth Error: App credentials not configured.', ['user_id' => $user_id]);
            wp_redirect(add_query_arg('auth_error', 'config_missing', admin_url('admin.php?page=dm-api-keys')));
            exit;
        }

        // Re-instantiate self with app credentials to handle the callback
        $oauth_handler = new self($app_id, $app_secret, $this->logger);
