...
class ModalConfigAjax {
    ...
    public function handle_get_modal_content() {
        // Verify nonce
        if ( ! wp_verify_nonce( $_POST['nonce'] ?? '', 'dm_get_modal_content_nonce' ) ) {
            wp_send_json_error( array( 'message' => __( 'Security check failed.', 'data-machine' ) ) );
        }
    ...
    public function handle_save_modal_config() {
        // Verify nonce
        if ( ! wp_verify_nonce( $_POST['nonce'] ?? '', 'dm_save_modal_config_nonce' ) ) {
            wp_send_json_error( array( 'message' => __( 'Security check failed.', 'data-machine' ) ) );
        }
