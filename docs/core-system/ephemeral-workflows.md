...
Ephemeral workflows (@since v0.8.0) allow you to execute a series of steps without first saving them as a Pipeline or Flow in the database. This is ideal for one-off tasks, programmatic triggers, or AI-generated workflows.

## How It Works

When an ephemeral workflow is triggered via the `/execute` REST endpoint, the system:
...
3.  **Sentinel Values**: Sets `flow_id = 0` and `pipeline_id = 0` within these configurations. These IDs act as markers for the engine to know it is dealing with an ephemeral execution.
...
- **AI Chatbot Execution**: When the Data Machine chat agent suggests a sequence of actions, it can trigger them immediately as an ephemeral workflow.
...
- **No Persistence**: You cannot "edit" an ephemeral workflow once it has started.
- **No Automatic Retries**: If an ephemeral workflow fails, it must be re-submitted via the API.
- **Sentinel IDs**: Because `flow_id` is 0, certain features that rely on flow-specific settings (like flow-level overrides) must be included directly in the `handler_config` within the request.
