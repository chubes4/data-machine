        ...
        onApiKeyChange: function(componentId, apiKey) {
            ...
            instance.apiKeyTimeout = setTimeout(() => {
                ...
                this.autoSaveSettings(componentId)
                    .then(() => {
                        // Ensure provider selection is maintained after save
                        if (selectedProvider && elements.providerSelect) {
                            elements.providerSelect.value = selectedProvider;
                            console.log('AI HTTP Client: Provider selection restored to:', selectedProvider);
                        }
                    ...
                    .catch(error => {
                        ...
                        // Restore provider selection even on error
                        if (selectedProvider && elements.providerSelect) {
                            elements.providerSelect.value = selectedProvider;
                            console.log('AI HTTP Client: Provider selection restored after error to:', selectedProvider);
                        }
        ...
        loadProviderSettings: function(componentId, provider) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (elements.temperatureInput) {
                        // Only update if field is empty to preserve user input
                        if (!elements.temperatureInput.value.trim()) {
                            elements.temperatureInput.value = settings.temperature || '0.5';
                            this.updateTemperatureValue(componentId, settings.temperature || '0.5');
                        }
                    }
                    
                    if (elements.systemPromptTextarea) {
                        // Only update if field is empty to preserve user input
                        if (!elements.systemPromptTextarea.value.trim()) {
