...
function dm_register_utility_filters() {
    ...
    // Hooks into AI HTTP Client's ai_config filter to provide step configuration from pipeline database
    // Usage: Applied automatically when step_id is provided in ai_request filter
    add_filter('ai_config', function($library_config, $step_id = null) {
        // If no step_id provided, return library config unchanged (shared API keys only)
        if (!$step_id) {
            return $library_config;
        }
        
        // Get step configuration from Data Machine pipeline database
        $step_config = apply_filters('dm_get_flow_step_config', [], $step_id);
        if (empty($step_config)) {
            do_action('dm_log', 'debug', 'AI Config: No step configuration found', ['step_id' => $step_id]);
            return $library_config;
        }
        
        // Extract AI configuration from step config
        $selected_provider = $step_config['selected_provider'] ?? 'openai';
        $temperature = isset($step_config['temperature']) ? (float) $step_config['temperature'] : null;
        $system_prompt = $step_config['system_prompt'] ?? '';
        
        // Get provider-specific model from step config
        $step_model = null;
        if (isset($step_config['model'])) {
            $step_model = $step_config['model'];
        } elseif (isset($step_config[$selected_provider . '_model'])) {
            $step_model = $step_config[$selected_provider . '_model'];
        }
        
        // Start with library config (shared API keys) as base
        $enhanced_config = is_array($library_config) ? $library_config : [];
        
        // Enhance configuration for all providers with step-specific data
        foreach ($enhanced_config as $provider_name => $provider_data) {
            if (is_array($provider_data) && isset($provider_data['api_key'])) {
                // Add step-specific model if this is the selected provider
                if ($provider_name === $selected_provider && $step_model) {
                    $enhanced_config[$provider_name]['model'] = $step_model;
                }
                
                // Add step-specific temperature if this is the selected provider
                if ($provider_name === $selected_provider && $temperature !== null) {
                    $enhanced_config[$provider_name]['temperature'] = $temperature;
                }
                
                // Add step-specific system_prompt if this is the selected provider
                if ($provider_name === $selected_provider && !empty($system_prompt)) {
                    $enhanced_config[$provider_name]['system_prompt'] = $system_prompt;
                }
                
                // Add selected_provider flag to all providers
                $enhanced_config[$provider_name]['is_selected'] = ($provider_name === $selected_provider);
            }
        }
        
        do_action('dm_log', 'debug', 'AI Config: Enhanced configuration with step data', [
            'step_id' => $step_id,
            'selected_provider' => $selected_provider,
            'has_temperature' => ($temperature !== null),
            'has_system_prompt' => !empty($system_prompt),
            'has_model' => !empty($step_model),
            'providers_count' => count($enhanced_config)
        ]);
        
        return $enhanced_config;
    }, 15, 2); // Priority 15 to override library's priority 10
