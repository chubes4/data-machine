...
class ChatTitleGenerator {
	...
	public function generate(string $session_id): bool {
		...
		if (!$session) {
			do_action('datamachine_log', 'error', 'Chat title generation failed - session not found', [
		...
		if ($success) {
			do_action('datamachine_log', 'debug', 'Chat session title generated', [
				'session_id' => $session_id,
				'title' => $title,
	...
	private function generateAITitle(string $first_user_message, ?string $first_assistant_response, array $session): ?string {
		...
		if (empty($provider) || empty($model)) {
			do_action('datamachine_log', 'warning', 'Chat title AI generation skipped - no default provider/model', [
		...
		try {
			$response = apply_filters('chubes_ai_request', $request, $provider, null, [], null, [
				'agent_type' => 'system',
				'purpose' => 'chat_title_generation'
			]);

			if (isset($response['error'])) {
				do_action('datamachine_log', 'error', 'Chat title AI generation failed', [
					'session_id' => $session['session_id'] ?? '',
			...
			do_action('datamachine_log', 'error', 'Chat title AI generation exception', [
				'session_id' => $session['session_id'] ?? '',
 ...
 */
add_action('datamachine_generate_chat_title', function($session_id) {
