...
class Bluesky extends BaseOutputHandler {
    ...
    public function handle( string $ai_output_string, array $module_job_config, ?int $user_id, array $input_metadata ): array {
        ...
        $parser = $this->load_ai_response_parser($ai_output_string, 'Bluesky Output');
        if (is_wp_error($parser)) {
            return $parser; // Return the WP_Error
        }
    ...
    private function _detect_link_facets(string $text): array {
        ...
        // Mention and tag detection can be extended through the dm_bluesky_facets filter
        // for custom mention (@username) and hashtag (#tag) processing as needed.
