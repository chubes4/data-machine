...
class WordPress extends BaseOutputHandler {
    ...
    private function publish_remote(string $ai_output_string, array $config, ?int $user_id, array $input_metadata, array $module_job_config): array {
        ...
        if (empty($location_id)) {
            ...
            $this->logger->error($error_message, ['config' => $config]);
            return [
                'success' => false,
                'error' => $error_message
            ];
        ...
        $db_locations = $this->get_db_remote_locations();
        if (!$db_locations) {
            return [
                'success' => false,
            ...
            if (!empty($parsed_data['tags'])) {
                ...
                if (!empty($first_tag_name)) {
                    ...
                    if (count($parsed_data['tags']) > 1) {
                        $this->logger->info("Remote Publish: Instruct mode - Sending only first tag '{$first_tag_name}'. AI provided: " . implode(', ', $parsed_data['tags']), ['location_id' => $location_id]);
                    }
                } else {
                    $assigned_tag_names = [];
                }
        ...
        if (is_wp_error($response)) {
            ...
            $this->logger->error($error_message, ['location_id' => $location_id, 'payload' => $payload]);
            return [
                'success' => false,
                'error' => $error_message
            ];
        ...
        if ($response_code < 200 || $response_code >= 300) {
            ...
            $this->logger->error($error_message, ['location_id' => $location_id, 'response_code' => $response_code, 'response_body' => $response_body]);
            return [
                'success' => false,
                'error' => $error_message
            ];
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            $this->logger->error($error_message, ['location_id' => $location_id, 'response_body' => $response_body]);
            return [
                'success' => false,
                'error' => $error_message
            ];
        ...
        if (empty($response_data['success']) || $response_data['success'] !== true) {
            ...
            $this->logger->error($error_message, ['location_id' => $location_id, 'response_data' => $response_data]);
            return [
                'success' => false,
                'error' => $error_message
            ];
