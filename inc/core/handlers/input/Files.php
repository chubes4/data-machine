...
class Files extends BaseInputHandler {
    ...
    private function read_text_file_content(string $file_path, string $mime_type): ?string {
        try {
            ...
            if ($file_size === false) {
                $this->logger?->error('Files Input: Cannot determine file size.', ['file_path' => $file_path]);
                return null;
            }
            ...
            if ($file_size > $max_file_size) {
                $this->logger?->error('Files Input: File too large to process safely.', [
                    'file_path' => $file_path,
            ...
            try {
                ...
                $this->logger?->error('Files Input: Error reading file content.', [
                    'file_path' => $file_path,
                    'error' => $e->getMessage()
                ]);
                return null;
