...
class WordPress extends BaseInputHandler {
    ...
    public function get_input_data(object $module, array $source_config, int $user_id): DataPacket {
        ...
        switch ($source_type) {
            case 'local':
                return $this->fetch_local_data($module_id, $config, $user_id);
            
            case 'remote_rest':
                return $this->fetch_remote_rest_data($module_id, $config, $user_id);
            
            case 'remote_airdrop':
                return $this->fetch_remote_airdrop_data($module_id, $config, $user_id);
     ...
     */
    private function fetch_local_data(int $module_id, array $config, int $user_id): DataPacket {
        ...
        if (empty($posts)) {
            return new DataPacket('No Data', 'No posts found matching the criteria', 'wordpress_local');
        }
        ...
        foreach ($posts as $post) {
            ...
            $post_id = $post->ID;
            if ($this->check_if_processed($module_id, 'wordpress_local', $post_id)) {
                continue;
            }
        ...
        if (empty($eligible_items_packets)) {
            return new DataPacket('No Data', 'No new posts found matching the criteria', 'wordpress_local');
        }
     ...
     * @throws Exception If data cannot be retrieved.
     */
    private function fetch_remote_rest_data(int $module_id, array $config, int $user_id): DataPacket {
        ...
        $this->logger?->info('WordPress REST Input: Initial fetch URL', ['url' => $next_page_url, 'module_id' => $module_id]);

        while ($next_page_url && count($eligible_items_packets) < $process_limit && $pages_fetched < $max_pages) {
            $pages_fetched++;
            $this->logger?->debug('WordPress REST Input: Fetching page', ['page' => $pages_fetched, 'url' => $next_page_url, 'module_id' => $module_id]);

            // Use HTTP service
            $http_response = $this->http_service->get($next_page_url, [], 'WordPress REST API');
            ...
            $response_data = $this->http_service->parse_json($http_response['body'], 'WordPress REST API');
            ...
            foreach ($items as $item) {
                ...
                if ($this->check_if_processed($module_id, 'wordpress_remote_rest', $current_item_id)) {
                    continue;
                }
                ...
                $this->logger?->debug('WordPress REST Input: Adding eligible item', ['item_id' => $current_item_id, 'title' => $title, 'module_id' => $module_id]);
        ...
        if (empty($eligible_items_packets)) {
            ...
            return new DataPacket('No Data', 'No new items found matching the criteria from the API endpoint', 'wordpress_remote_rest');
        }
     ...
     * @throws Exception If data cannot be retrieved.
     */
    private function fetch_remote_airdrop_data(int $module_id, array $config, int $user_id): DataPacket {
        ...
        while (count($eligible_items_packets) < $process_limit && $current_page <= $max_pages && !$hit_time_limit_boundary) {
            ...
            foreach ($posts_data as $post) {
                ...
                if ($this->check_if_processed($module_id, 'wordpress_remote_airdrop', $current_item_id)) {
                    continue;
                }
        ...
        if (empty($eligible_items_packets)) {
            return new DataPacket('No Data', 'No new items found matching the criteria from the helper API endpoint', 'wordpress_remote_airdrop');
        }
    ...
    private function extract_image_url(int $post_id, string $content): ?string {
        ...
        if (empty($image_url) && !empty($content)) {
            if (preg_match('/<img.*?src=[\'"](.*?)[\'"].*?>/i', $content, $matches)) {
                ...
                if (filter_var($first_image_src, FILTER_VALIDATE_URL)) {
                    $image_url = $first_image_src;
                    $this->logger?->debug('WordPress Input: Using first image from content as fallback.', ['found_url' => $image_url, 'post_id' => $post_id]);
                }
            }
        }
