<?php
/**
 * Base Pipeline Step Class - Pure Filter-Based Architecture
 * 
 * Provides common functionality and helper methods for pipeline steps.
 * This class is completely independent with no interface requirements.
 * External plugins can extend from this or create completely independent classes.
 * All step detection is capability-based, not inheritance-based.
 * 
 * PURE CAPABILITY DETECTION:
 * - No interfaces required
 * - No abstract methods to implement  
 * - External classes need only provide execute() method
 * - Supports alternative method names (run, process)
...
class BasePipelineStep {
     ...
     */
    protected function store_step_data_packet(int $job_id, \DataMachine\Core\DataPacket $data_packet): bool {
     ...
     */
    protected function get_previous_step_data_packet(int $job_id): ?\DataMachine\Core\DataPacket {
    ...
    protected function get_all_previous_data_packets(int $job_id): array {
        ...
        for ($i = 0; $i < $current_position; $i++) {
            ...
            try {
                $packet = \DataMachine\Core\DataPacket::fromJson($json_data);
                if ($packet) {
                    $data_packets[] = $packet;
                }
            } catch (\Exception $e) {
     ...
     *
     * @return object The logger service.
     */
     ...
     *
     * @return object The AI HTTP client service.
     */
    protected function get_ai_client() {
        return apply_filters('dm_get_ai_http_client', null);
    }
    ...
    protected function get_prompt_builder() {
        return apply_filters('dm_get_prompt_builder', null);
    }
    
    /**
     * Get the job status manager service.
     *
     * @return object The job status manager service.
     */
    protected function get_job_status_manager() {
        return apply_filters('dm_get_job_status_manager', null);
    }
    ...
    protected function get_db_jobs() {
        return apply_filters('dm_get_db_jobs', null);
    }
