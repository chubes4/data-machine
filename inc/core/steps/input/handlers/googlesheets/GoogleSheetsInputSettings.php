 ...
 * @subpackage Data_Machine/handlers/input/googlesheets
 * @since      NEXT_VERSION
 */
...
class GoogleSheetsInputSettings {
    ...
    /**
     * Get settings fields for Google Sheets input handler.
     *
    ...
    public static function get_fields(array $current_config = []): array {
        return [
            'googlesheets_input_spreadsheet_id' => [
                'type' => 'text',
            ...
            'googlesheets_input_worksheet_name' => [
                'type' => 'text',
            ...
            'googlesheets_input_has_header_row' => [
                'type' => 'checkbox',
    ...
    /**
     * Sanitize Google Sheets input handler settings.
     *
    ...
    public static function sanitize(array $raw_settings): array {
        ...
        $spreadsheet_id = sanitize_text_field($raw_settings['googlesheets_input_spreadsheet_id'] ?? '');
        $sanitized['googlesheets_input_spreadsheet_id'] = preg_replace('/[^a-zA-Z0-9_-]/', '', $spreadsheet_id);
        
        // Sanitize worksheet name
        $sanitized['googlesheets_input_worksheet_name'] = sanitize_text_field($raw_settings['googlesheets_input_worksheet_name'] ?? 'Sheet1');
        
        // Sanitize cell range with A1 notation validation
        $cell_range = sanitize_text_field($raw_settings['googlesheets_input_cell_range'] ?? 'A1:Z1000');
        if (preg_match('/^[A-Z]+\d+:[A-Z]+\d+$/', $cell_range)) {
            $sanitized['googlesheets_input_cell_range'] = $cell_range;
        } else {
            $sanitized['googlesheets_input_cell_range'] = 'A1:Z1000'; // Default fallback
        }
        
        // Header row option
        $sanitized['googlesheets_input_has_header_row'] = !empty($raw_settings['googlesheets_input_has_header_row']);
        
        // Row limit with bounds checking
        $row_limit = absint($raw_settings['googlesheets_input_row_limit'] ?? 100);
        $sanitized['googlesheets_input_row_limit'] = max(1, min(1000, $row_limit));
    ...
    public static function get_defaults(): array {
        return [
            'googlesheets_input_spreadsheet_id' => '',
            'googlesheets_input_worksheet_name' => 'Sheet1',
            'googlesheets_input_cell_range' => 'A1:Z1000',
            'googlesheets_input_has_header_row' => true,
            'googlesheets_input_row_limit' => 100
        ];
    }
