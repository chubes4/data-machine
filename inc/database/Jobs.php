...
class Jobs {
    ...
    public function create_job(array $job_data): int|false {
        ...
        // Skip job creation if no pipeline configuration exists
        if ( empty( $step_sequence ) ) {
            $logger = apply_filters('dm_get_logger', null);
            if ( $logger ) {
                $logger->warning( 'Cannot create job - no pipeline configuration found', [
                    'module_id' => $module_id,
        ...
        $data = array(
            ...
            'current_step_name' => $step_sequence[0], // Set first step as current
    ...
    private function build_step_sequence_for_project( ?int $project_id ): array {
        ...
        try {
            ...
            $logger = apply_filters('dm_get_logger', null);
            if ( $logger ) {
                $logger->warning( 'Error building step sequence for project', [
                    'project_id' => $project_id,
                    'error' => $e->getMessage()
                ] );
            }
