...
class LocalSearch {
    ...
    public function handle_tool_call(array $parameters, array $tool_def = []): array {
        ...
        $post_types = $this->normalizePostTypes($parameters['post_types'] ?? ['post', 'page']);
        ...
        return $this->buildResponse([], $raw_query, $post_types, 'none');
    }
    ...
    private function searchByTitle(string $query, array $post_types, int $limit): array {
        ...
        $sql = $wpdb->prepare(
            "SELECT ID FROM {$wpdb->posts} 
             ...
             LIMIT %d",
            ...array_merge($post_types, ['%' . $wpdb->esc_like($query) . '%', $limit])
        );
