...
class JobAbilities {
	...
	private function registerAbilities(): void {
		$register_callback = function () {
			...
			$this->registerRecoverStuckJobs();
			$this->registerGetJobsSummary();
		};
	...
	/**
	 * Register datamachine/recover-stuck-jobs ability.
	 */
	private function registerRecoverStuckJobs(): void {
		wp_register_ability(
			...
			array(
				...
				'execute_callback'    => array( $this, 'executeRecoverStuckJobs' ),
				'permission_callback' => array( $this, 'checkPermission' ),
				'meta'                => array( 'show_in_rest' => true ),
			)
		);
	...
	/**
	 * Register datamachine/get-jobs-summary ability.
	 */
	private function registerGetJobsSummary(): void {
		wp_register_ability(
			...
			array(
				...
				'execute_callback'    => array( $this, 'executeGetJobsSummary' ),
				'permission_callback' => array( $this, 'checkPermission' ),
				'meta'                => array( 'show_in_rest' => true ),
			)
		);
	...
	public function executeRecoverStuckJobs( array $input ): array {
		...
		foreach ( $stuck_jobs as $job ) {
			...
			if ( ! $status || ! \DataMachine\Core\JobStatus::isStatusFinal( $status ) ) {
				++$skipped;
				$jobs[] = array(
