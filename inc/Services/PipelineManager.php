...
class PipelineManager {
	 ...
	 */
	public function create( string $name, array $options = array() ): ?array {
		if ( ! current_user_can( 'manage_options' ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Insufficient permissions for pipeline creation',
				array(
	 ...
	 * @return array|null Complete pipeline data on success, null on failure
	 */
	public function createWithSteps( string $name, array $steps, array $options = array() ): ?array {
		if ( ! current_user_can( 'manage_options' ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Insufficient permissions for pipeline creation with steps',
				array(
					'user_id'        => get_current_user_id(),
					'attempted_name' => $name,
				)
		...
		if ( ! $pipeline_id ) {
			do_action( 'datamachine_log', 'error', 'Failed to create pipeline in createWithSteps', array( 'pipeline_name' => $pipeline_name ) );
			return null;
		}
		...
		do_action(
			'datamachine_log',
			'info',
			'Pipeline created successfully with steps',
			array(
				'pipeline_id'   => $pipeline_id,
				'pipeline_name' => $pipeline_name,
	...
	public function update( int $pipeline_id, array $data ): bool {
		if ( ! current_user_can( 'manage_options' ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Insufficient permissions for pipeline update',
				array(
					'user_id'     => get_current_user_id(),
					'pipeline_id' => $pipeline_id,
				)
	...
	public function delete( int $pipeline_id ): array|WP_Error {
		...
		if ( ! isset( $pipeline['pipeline_name'] ) || empty( trim( $pipeline['pipeline_name'] ) ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Cannot delete pipeline - missing or empty pipeline name',
				array(
					'pipeline_id' => $pipeline_id,
				)
			);
		...
		foreach ( $affected_flows as $flow ) {
			if ( ! isset( $flow['flow_id'] ) || empty( $flow['flow_id'] ) ) {
				do_action(
					'datamachine_log',
					'error',
					'Flow data missing flow_id during pipeline deletion',
					array(
						'pipeline_id' => $pipeline_id,
						'flow'        => $flow,
					)
