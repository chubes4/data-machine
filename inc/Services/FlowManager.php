...
class FlowManager {
	 ...
	 */
	public function create( int $pipeline_id, string $name, array $options = array() ): ?array {
		if ( ! current_user_can( 'manage_options' ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Insufficient permissions for flow creation',
				array(
		...
		$scheduling_config = $options['scheduling_config'] ?? array( 'interval' => 'manual' );
		$flow_config       = $options['flow_config'] ?? array();
	...
	public function delete( int $flow_id ): bool {
		if ( ! current_user_can( 'manage_options' ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Insufficient permissions for flow deletion',
				array(
					'user_id' => get_current_user_id(),
					'flow_id' => $flow_id,
				)
	...
	public function copyToPipeline(
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Insufficient permissions for flow copy',
				array(
					'user_id'            => get_current_user_id(),
		...
		$requested_scheduling_config = $options['scheduling_config'] ?? ( $source_flow['scheduling_config'] ?? array() );
		...
		$new_flow_config = $this->buildCopiedFlowConfig(
			...
			$new_flow_id,
			$target_pipeline_id,
			$options['step_config_overrides'] ?? array()
		);

		// Update flow with the new config
		$update_success = $this->db_flows->update_flow(
			$new_flow_id,
			array(
		...
		if ( ! $update_success ) {
			do_action(
				'datamachine_log',
				'error',
				'Failed to update flow config during copy',
				array(
	 ...
	 * @return bool Success status
	 */
	public function syncStepsToFlow( int $flow_id, int $pipeline_id, array $steps, array $pipeline_config = array() ): bool {
		$flow = $this->db_flows->get_flow( $flow_id );
		if ( ! $flow ) {
