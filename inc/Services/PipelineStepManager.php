	...
	public function add( int $pipeline_id, string $step_type ): ?string {
		if ( ! current_user_can( 'manage_options' ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Insufficient permissions for step creation',
				array(
		...
		if ( $pipeline_id <= 0 ) {
			do_action(
				'datamachine_log',
				'error',
				'Pipeline ID is required for step creation',
				array(
					'pipeline_id' => $pipeline_id,
				)
			);
		...
		if ( empty( $step_type ) ) {
			do_action(
				'datamachine_log',
				'error',
				'Step type is required for step creation',
				array(
					'pipeline_id' => $pipeline_id,
				)
			);
		...
		foreach ( $flows as $flow ) {
			$this->flow_manager->syncStepsToFlow( $flow['flow_id'], $pipeline_id, array( $new_step ), $pipeline_config );
		}
	...
	public function updateSystemPrompt( string $pipeline_step_id, string $system_prompt ): bool {
		...
		if ( empty( $step_config ) || empty( $step_config['pipeline_id'] ) ) {
			do_action(
				'datamachine_log',
				'error',
				'System prompt update failed - pipeline step not found',
				array(
					'pipeline_step_id' => $pipeline_step_id,
				)
			);
		...
		if ( ! $target_pipeline ) {
			do_action(
				'datamachine_log',
				'error',
				'System prompt update failed - pipeline not found',
				array(
					'pipeline_id'      => $pipeline_id,
					'pipeline_step_id' => $pipeline_step_id,
				)
		...
		$pipeline_config = $target_pipeline['pipeline_config'] ?? array();
		...
		if ( ! $success ) {
			do_action(
				'datamachine_log',
				'error',
				'System prompt update failed - database update error',
				array(
					'agent_type'       => 'system',
