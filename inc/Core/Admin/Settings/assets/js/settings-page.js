    ...
    window.dmSettingsPage = {
        ...
        bindEvents: function() {
            ...
            document.addEventListener('click', (e) => {
                if (e.target.classList.contains('datamachine-tool-config-save')) {
                    this.handleToolConfigSave(e);
                }
            });

            // Cache clearing handler
            const clearCacheBtn = document.getElementById('datamachine-clear-cache-btn');
            if (clearCacheBtn) {
                clearCacheBtn.addEventListener('click', this.handleClearCache.bind(this));
            }

            // Tab navigation handlers
            const tabLinks = document.querySelectorAll('.datamachine-nav-tab-wrapper .nav-tab');
            tabLinks.forEach(tab => {
                tab.addEventListener('click', this.handleTabClick.bind(this));
            });

            // Form submission handler to preserve tab state
            const settingsForm = document.querySelector('.datamachine-settings-form');
            if (settingsForm) {
        ...
        handleToolConfigSave: function(e) {
            ...
            // Find the form
            const form = document.getElementById(`datamachine-${toolId}-config-form`);

            if (!form) {
                console.error('Tool config save: Form not found for tool:', toolId);
                return;
            }
            ...
            requiredFields.forEach(field => {
                ...
                if (!configData[fieldName] || configData[fieldName].trim() === '') {
                    const label = field.closest('tr').querySelector('label').textContent || fieldName;
            ...
            button.textContent = datamachineSettings.strings.saving || 'Saving...';
            button.disabled = true;

            // Use REST API endpoint
            const restUrl = wpApiSettings.root + 'datamachine/v1/settings/tools/' + toolId;
        ...
        handleClearCache: function(e) {
            ...
            const button = e.target;
            const result = document.getElementById('datamachine-cache-clear-result');
            ...
            button.disabled = true;
            result.classList.remove('datamachine-hidden', 'notice-success', 'notice-error');
            result.textContent = '';
        ...
        showError: function(message) {
            const modalBody = document.querySelector('.datamachine-modal[aria-hidden="false"] .datamachine-modal-body');
            ...
            const existingErrors = modalBody.querySelectorAll('.notice-error');
        ...
        initTabManager: function() {
            this.tabManager = {
                ...
                showTab: function(tab) {
                    ...
                    const tabContents = document.querySelectorAll('.datamachine-tab-content');
                    ...
                    // Show selected tab
                    const selectedTab = document.getElementById('datamachine-tab-' + tab);
                    if (selectedTab) {
                    ...
                    const navTabs = document.querySelectorAll('.datamachine-nav-tab-wrapper .nav-tab');
                    navTabs.forEach(navTab => {
                        navTab.classList.remove('nav-tab-active');
                        if (navTab.getAttribute('href').includes('tab=' + tab)) {
        ...
        handleFormSubmit: function(e) {
            ...
            const form = e.target;
            const existingTabInput = form.querySelector('input[name="datamachine_active_tab"]');
            if (existingTabInput) {
