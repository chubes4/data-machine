			...
			<div className="datamachine-chat-session-list__content">
				{isLoading ? (
					<div className="datamachine-chat-session-list__loading">
						...
						<span>{__('Loading conversations...', 'data-machine')}</span>
					</div>
				) : sessions.length === 0 ? (
					<div className="datamachine-chat-session-list__empty">
						<p>{__('No conversations yet.', 'data-machine')}</p>
						<p>{__('Start a new conversation to begin.', 'data-machine')}</p>
					</div>
				) : (
								...
								>
									...
									<span className="datamachine-chat-session-list__item-meta">
										...
										{session.message_count > 0 && (
											<> &middot; {session.message_count} {session.message_count === 1 ? __('message', 'data-machine') : __('messages', 'data-machine')}</>
										)}
									</span>
								</button>
								<Button
									...
									label={__('Delete conversation', 'data-machine')}
