...
export function useChatQueryInvalidation() {
	...
	const invalidateFromToolCalls = useCallback((toolCalls, selectedPipelineId) => {
		...
		for (const toolCall of toolCalls) {
			...
			if (FLOW_TOOLS.includes(toolName)) {
				const pipelineId = params.pipeline_id || params.target_pipeline_id;

				if (pipelineId) {
					pipelineIdsToInvalidate.add(normalizeId(pipelineId));
				} else if (selectedPipelineId) {
					pipelineIdsToInvalidate.add(normalizeId(selectedPipelineId));
				}
			}
