...
import { usePipelinesForDropdown, useFlowsForDropdown, useClearProcessedItems } from '../../queries/jobs';
...
const ClearProcessedForm = () => {
	...
	const { data: pipelines = [], isLoading: pipelinesLoading } = usePipelinesForDropdown();
	const { data: flows = [], isLoading: flowsLoading } = useFlowsForDropdown( pipelineId );
	...
	const handleSubmit = useCallback( ( e ) => {
		...
		if ( ! clearType ) {
			setNotice( { type: 'warning', message: __( 'Please select a clear type', 'data-machine' ) } );
			return;
		}
		...
		if ( clearType === 'pipeline' ) {
			targetId = pipelineId;
			if ( ! targetId ) {
				setNotice( { type: 'warning', message: __( 'Please select a pipeline', 'data-machine' ) } );
				return;
			}
			confirmMessage = __( 'Are you sure you want to clear all processed items for ALL flows in this pipeline? This will allow all items to be reprocessed.', 'data-machine' );
		} else if ( clearType === 'flow' ) {
			targetId = flowId;
			if ( ! targetId ) {
				setNotice( { type: 'warning', message: __( 'Please select a flow', 'data-machine' ) } );
				return;
			}
			confirmMessage = __( 'Are you sure you want to clear all processed items for this flow? This will allow all items to be reprocessed.', 'data-machine' );
		}
	...
	const flowOptions = [
		{ label: pipelineId ? __( '— Select a Flow —', 'data-machine' ) : __( '— Select a Pipeline First —', 'data-machine' ), value: '' },
		...flows.map( ( f ) => ( {
			label: f.flow_name,
	...
	const clearTypeOptions = [
		...
		{ label: __( 'Entire Pipeline (all flows)', 'data-machine' ), value: 'pipeline' },
	...
	return (
		<div className="datamachine-admin-section">
			...
			<p className="description">
				{ __( 'Clear processed item records to allow reprocessing during testing and development. This is useful when iteratively refining prompts and configurations.', 'data-machine' ) }
			</p>
			...
			<form onSubmit={ handleSubmit } className="datamachine-admin-form">
				...
				{ ( clearType === 'pipeline' || clearType === 'flow' ) && (
					<div className="datamachine-form-field">
						...
						{ clearType === 'pipeline' && (
							<p className="description">
								{ __( 'All processed items for ALL flows in this pipeline will be cleared.', 'data-machine' ) }
							</p>
						) }
					</div>
				) }
				...
				{ clearType === 'flow' && (
					<div className="datamachine-form-field">
						...
						/>
						<p className="description">
							{ __( 'All processed items for this specific flow will be cleared.', 'data-machine' ) }
						</p>
					</div>
				) }
