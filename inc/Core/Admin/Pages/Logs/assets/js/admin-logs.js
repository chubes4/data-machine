...
( function () {
	...
	function initLogsPage() {
		...
		const clearLogsBtn = document.querySelector(
			'.datamachine-clear-logs-btn'
		);
		if ( clearLogsBtn ) {
		...
		if ( copyBtn ) {
			copyBtn.addEventListener( 'click', function ( e ) {
				...
				const targetSelector =
					copyBtn.getAttribute( 'data-copy-target' );
		...
		const loadFullBtn = document.querySelector(
			'.datamachine-load-full-logs'
		);
		if ( loadFullBtn ) {
	...
	function handleFullLogLoad() {
		const button = document.getElementById(
			'datamachine-load-full-logs-btn'
		);
		const logViewer = document.querySelector( '.datamachine-log-viewer' );
		const sectionTitle = document.querySelector(
			'.datamachine-log-section-title'
		);
		const currentMode = logViewer
			? logViewer.getAttribute( 'data-current-mode' )
			: null;
			...
			.then( function ( response ) {
				...
				if ( sectionTitle ) {
					sectionTitle.textContent =
						'Full Log File (' + response.total_lines + ' entries)';
				}
	...
	function showStatusMessage( message, type ) {
		const statusMessage = document.querySelector(
			'.datamachine-log-status-message'
		);
		...
		statusMessage.classList.remove(
			'datamachine-status-success',
			'datamachine-status-error',
			'datamachine-status-info'
		);
