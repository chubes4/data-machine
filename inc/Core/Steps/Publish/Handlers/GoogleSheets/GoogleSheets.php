...
class GoogleSheets extends PublishHandler {
	...
	protected function executePublish( array $parameters, array $handler_config ): array {
		$this->log(
			'debug',
			'Google Sheets Tool: Handling tool call',
			array(
		...
		$this->log(
			'debug',
			'Google Sheets Tool: Using handler configuration',
			array(
			...
			$engine = $parameters['engine'] ?? new EngineData( array(), $job_id );
		if ( ! $engine instanceof EngineData ) {
			$engine = new EngineData( array(), $job_id );
		}
		...
		if ( is_wp_error( $sheets_service ) ) {
			return $this->errorResponse(
				...
				array( 'error_code' => $sheets_service->get_error_code() ),
				'critical'
			);
		}
		...
		try {
			...
			$this->log(
				'debug',
				'Google Sheets Tool: Data appended successfully',
				array(
					'spreadsheet_id' => $spreadsheet_id,
	...
	private function append_to_sheet( $sheets_service, string $spreadsheet_id, string $worksheet_name, array $row_data ) {
		...
		try {
			...
			$body = array(
				'values' => array( $row_data ), // Single row of data
			);
