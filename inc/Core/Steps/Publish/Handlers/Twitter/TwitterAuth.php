...
class TwitterAuth {
    ...
    const TEMP_TOKEN_SECRET_TRANSIENT_PREFIX = 'datamachine_twitter_req_secret_'; // Prefix + request_token
    ...
    public function get_authorization_url(): string {
        ...
        try {
            ...
            if ($connection->getLastHttpCode() != 200 || !isset($request_token['oauth_token']) || !isset($request_token['oauth_token_secret'])) {
                ...
                do_action('datamachine_log', 'error', 'Twitter OAuth Error: ' . $error_message, [
            ...
            do_action('datamachine_log', 'error', 'Twitter OAuth Exception: ' . $e->getMessage(), [
                'handler' => 'twitter',
                'operation' => 'get_authorization_url'
            ]);
            return '';
    ...
    public function handle_oauth_callback() {
        ...
        if (!current_user_can('manage_options')) {
             wp_redirect(add_query_arg('auth_error', 'twitter_permission_denied', admin_url('admin.php?page=datamachine-pipelines')));
             exit;
        }
        ...
        if (!empty($denied)) {
            ...
            do_action('datamachine_log', 'warning', 'Twitter OAuth Warning: User denied access.', ['denied_token' => $denied]);
            wp_redirect(add_query_arg('auth_error', 'twitter_access_denied', admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }

        // Validate required parameters
        if (empty($oauth_token) || empty($oauth_verifier)) {
            do_action('datamachine_log', 'error', 'Twitter OAuth Error: Missing oauth_token or oauth_verifier in callback.');
            wp_redirect(add_query_arg('auth_error', 'twitter_missing_callback_params', admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }
        ...
        if (empty($oauth_token_secret)) {
            do_action('datamachine_log', 'error', 'Twitter OAuth Error: Request token secret missing or expired in transient.', ['oauth_token' => $oauth_token]);
            wp_redirect(add_query_arg('auth_error', 'twitter_token_secret_expired', admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }
        ...
        if (empty($api_key) || empty($api_secret)) {
            ...
            wp_redirect(add_query_arg('auth_error', 'twitter_missing_app_keys', admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }
        ...
        try {
            ...
            if ($connection->getLastHttpCode() != 200 || !isset($access_token_data['oauth_token']) || !isset($access_token_data['oauth_token_secret'])) {
                do_action('datamachine_log', 'error', 'Twitter OAuth Error: Failed to get access token.', [
                    'http_code' => $connection->getLastHttpCode(),
                    'response' => $connection->getLastBody()
                ]);
                wp_redirect(add_query_arg('auth_error', 'twitter_access_token_failed', admin_url('admin.php?page=datamachine-pipelines')));
                exit;
            }
            ...
            // Redirect on success
            wp_redirect(add_query_arg('auth_success', 'twitter', admin_url('admin.php?page=datamachine-pipelines')));
            exit;

        } catch (\Exception $e) {
            do_action('datamachine_log', 'error', 'Twitter OAuth Exception during callback: ' . $e->getMessage());
            wp_redirect(add_query_arg('auth_error', 'twitter_callback_exception', admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }
    }
