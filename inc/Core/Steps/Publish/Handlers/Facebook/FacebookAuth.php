...
class FacebookAuth {
    ...
    public function handle_callback(string $code, string $state): bool|\WP_Error {
        do_action('datamachine_log', 'debug', 'Handling Facebook OAuth callback.');
        ...
        // Facebook requires GET for token exchange - use datamachine_request filter
        $result = apply_filters('datamachine_request', null, 'GET', $token_url, [], 'Facebook OAuth');
        ...
        if ($http_code !== 200 || empty($data['access_token'])) {
            ...
            do_action('datamachine_log', 'error', 'Facebook OAuth Error: Token exchange failed.', ['http_code' => $http_code, 'response' => $body]);
        ...
        if (is_wp_error($long_lived_token_data)) {
            do_action('datamachine_log', 'error', 'Facebook OAuth Error: Failed to exchange for long-lived token.', [
        ...
        if (is_wp_error($page_credentials)) {
            do_action('datamachine_log', 'error', 'Facebook OAuth Error: Failed to fetch page credentials.', [
        ...
        if (!is_wp_error($profile_info)) {
            ...
            $user_profile_name = $profile_info['name'] ?? 'ErrorFetchingName';
             ...
             do_action('datamachine_log', 'warning', 'Facebook OAuth Warning: Failed to fetch user profile info.', [
        ...
        apply_filters('datamachine_store_oauth_account', $account_details, 'facebook');
        do_action('datamachine_log', 'debug',
    ...
    public function get_authorization_url(): string {
        $state = wp_create_nonce('datamachine_facebook_oauth_state');
        // Store state in admin-global transient for verification
        set_transient('datamachine_facebook_oauth_state', $state, 15 * MINUTE_IN_SECONDS);
    ...
    private function get_user_profile(string $access_token): array|\WP_Error {
        ...
        // Use datamachine_request filter for Facebook profile fetch
    ...
    private function exchange_for_long_lived_token(string $short_lived_token): array|\WP_Error {
        ...
        // Use datamachine_request filter for long-lived token exchange
    ...
    private function get_page_credentials(string $user_access_token): array|\WP_Error {
        ...
        $permissions_result = apply_filters('datamachine_request', null, 'GET', $permissions_url, [
            'headers' => [
        ...
        if ($permissions_result['success']) {
            ...
            do_action('datamachine_log', 'debug', 'Facebook permissions check', [
        ...
        do_action('datamachine_log', 'debug', 'Attempting to fetch Facebook pages from /me/accounts endpoint.', [
            'url' => $url
        ]);

        // Use datamachine_request filter for Facebook pages fetch
        $result = apply_filters('datamachine_request', null, 'GET', $url, [
            'headers' => [
                'Authorization' => 'Bearer ' . $user_access_token,
            ],
        ...
        do_action('datamachine_log', 'debug', 'Facebook pages API response', [
            'url' => $url,
            'http_code' => $http_code,
            'response_body' => $body,
        ...
        if ($http_code !== 200 || !isset($data['data'])) {
            ...
            do_action('datamachine_log', 'error', 'Facebook Page Fetch Error: API error.', [
                'http_code' => $http_code, 
                'response' => $body
            ]);
        ...
        do_action('datamachine_log', 'debug', 'Facebook: Processing first page from API response', [
    ...
    public function remove_account(): bool {
        ...
        if ($token) {
            // Attempt deauthorization with Facebook - use datamachine_request filter
    ...
    public function has_comment_permission(): bool {
        ...
        if (!$result['success']) {
            do_action('datamachine_log', 'error', 'Facebook: Failed to check comment permissions', [
                'error' => $result['error']
            ]);
            return false;
        }
        ...
        if (!isset($data['data']) || !is_array($data['data'])) {
            do_action('datamachine_log', 'error', 'Facebook: Invalid permission response format', [
                'response_data' => $data
            ]);
            return false;
        }
     ...
     * Called via the unified OAuth rewrite system at /datamachine-auth/facebook/.
     */
    public function handle_oauth_callback() {
        ...
        if (empty($state)) {
            do_action('datamachine_log', 'error', 'Facebook OAuth Error: Missing state parameter.');
            wp_redirect(add_query_arg('auth_error', 'facebook_missing_state', admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }

        $stored_state = get_transient('datamachine_facebook_oauth_state');
        delete_transient('datamachine_facebook_oauth_state');

        if (empty($stored_state) || false === wp_verify_nonce($state, 'datamachine_facebook_oauth_state')) {
            do_action('datamachine_log', 'error', 'Facebook OAuth Error: State mismatch or expired.');
            wp_redirect(add_query_arg('auth_error', 'facebook_state_mismatch', admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }
        ...
        if (!empty($error)) {
            ...
            do_action('datamachine_log', 'warning', 'Facebook OAuth Error in callback:', ['error' => $error, 'description' => $error_desc]);
            wp_redirect(add_query_arg('auth_error', $error, admin_url('admin.php?page=datamachine-pipelines')));
            exit;
        }

        // Check for required parameters
        if (empty($code)) {
            do_action('datamachine_log', 'error', 'Facebook OAuth Error: Missing code in callback.');
        ...
        if (empty($app_id) || empty($app_secret)) {
            do_action('datamachine_log', 'error', 'Facebook OAuth Error: App credentials not configured.');
        ...
        if (is_wp_error($result)) {
            ...
            do_action('datamachine_log', 'error', 'Facebook OAuth Callback Failed.', ['error_code' => $error_code, 'error_message' => $error_message]);
            // Redirect with a generic or specific error code
            wp_redirect(add_query_arg('auth_error', $error_code, admin_url('admin.php?page=datamachine-pipelines')));
        } else {
            do_action('datamachine_log', 'debug', 'Facebook OAuth Callback Successful.');
            wp_redirect(add_query_arg('auth_success', 'facebook', admin_url('admin.php?page=datamachine-pipelines')));
        }
        exit;
    }
}
