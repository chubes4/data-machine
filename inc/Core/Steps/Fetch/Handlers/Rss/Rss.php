...
class Rss extends FetchHandler {
	...
	protected function executeFetch( array $config, ExecutionContext $context ): array {
		...
		if (!$result['success']) {
			$context->log('error', 'Rss: Failed to fetch RSS feed.', [
				'error' => $result['error'],
				'feed_url' => $feed_url
			]);
			return [];
		...
		if (empty($feed_content)) {
			$context->log('error', 'Rss: RSS feed content is empty.', ['feed_url' => $feed_url]);
			return [];
		}
		...
		if ($xml === false) {
			...
			$context->log('error', 'Rss: Failed to parse RSS feed XML.', [
				'feed_url' => $feed_url,
		...
		if (isset($xml->channel->item)) {
			...
			$context->log('error', 'Rss: Unsupported feed format or no items found in feed.', ['feed_url' => $feed_url]);
			return [];
		}
		...
		foreach ($items as $item) {
			...
			if (empty($guid)) {
				$context->log('warning', 'Rss: Skipping item without GUID.', ['title' => $title]);
				continue;
			}
			...
			if ($pub_date) {
				...
				if ($item_timestamp !== false && !$this->applyTimeframeFilter($item_timestamp, $timeframe_limit)) {
					$context->log('debug', 'Rss: Skipping item outside timeframe.', [
						'guid' => $guid,
						'pub_date' => $pub_date
					]);
					continue;
			...
			if (!empty($enclosure_url)) {
				...
				if (strpos($mime_type, 'image/') === 0 && in_array($mime_type, ['image/jpeg', 'image/png', 'image/gif', 'image/webp'])) {
					...
					if ($download_result) {
						...
						$context->log('debug', 'Rss: Downloaded remote image for AI processing', [
							'guid' => $guid,
						...
						$context->log('warning', 'Rss: Failed to download remote image', [
							'guid' => $guid,
		...
		$context->log('debug', 'Rss: No eligible items found in RSS feed.', [
