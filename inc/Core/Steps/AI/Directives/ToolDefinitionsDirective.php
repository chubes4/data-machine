<?php
/**
 * Tool Definitions Directive
 * 
 * Injects available tools list, workflow context, and task completion guidance.
 * This is the main directive that tells the AI what tools are available and
 * provides complete pipeline workflow context for task orchestration.
 *
...
class ToolDefinitionsDirective {
    ...
    public static function inject($request, $provider_name, $streaming_callback, $tools, $pipeline_step_id = null): array {
        ...
        // Get flow context for enhanced workflow information
     ...
     * @param string|null $flow_step_id Flow step ID for flow-specific context
    ...
    public static function generate_dynamic_directive(array $tools, array $request = [], $pipeline_step_id = null, $flow_step_id = null): string {
        // Pipeline context is now mandatory
        self::require_pipeline_context($pipeline_step_id, __METHOD__);
        if (empty($pipeline_step_id)) {
        ...
        if (!empty($handler_tools)) {
            ...
            // Derive simple workflow status (tool-agnostic)
            $turn_count = 0;
    ...
    /**
     * Generate workflow context for AI directive using real flow-specific data
     * 
     ...
     * @param string|null $flow_step_id Current flow step ID for flow-specific context
    ...
    public static function generate_general_tool_directives(array $tools): string {
        ...
        // Critical directives for general tools
     ...
     * 
     * @param string $step_type Step type (fetch, ai, publish, update)
...
// Self-register with WordPress filter system (Priority 30 = third in order)
