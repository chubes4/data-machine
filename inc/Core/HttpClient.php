...
class HttpClient {
    ...
    private const ERROR_KEYS = ['message', 'error', 'error_description', 'detail'];
    ...
    public static function request(string $method, string $url, array $options = []): array {
        ...
        if (!in_array($method, self::VALID_METHODS)) {
            do_action('datamachine_log', 'error', 'HTTP Request: Invalid method', [
                'method' => $method,
                'context' => $context
            ]);
        ...
        $success_codes = self::SUCCESS_CODES[$method] ?? [200];
