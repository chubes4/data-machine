...
class ProjectPipelineConfigService {
    ...
    public function get_available_step_types(): array {
        $pipeline_step_registry = apply_filters('dm_get_pipeline_step_registry', null);
    ...
    /**
     * Create empty pipeline configuration for a project.
     * Projects start with no steps and users build their pipeline dynamically.
     *
     * @param int $project_id The ID of the project.
     * @param int $user_id    The ID of the user (for ownership verification).
     * @return bool True on success, false on failure.
     */
    public function create_empty_pipeline_configuration( int $project_id, int $user_id ): bool {
        $empty_config = [
            'steps' => []
        ];

        return $this->update_project_pipeline_steps( $project_id, $empty_config, $user_id );
    }
}