...
class ModuleConfigAjax {
    ...
    /**
     * Get Files handler via direct instantiation using pure filter-based architecture.
     * 
     * @return object|\WP_Error Files handler instance or WP_Error on failure
     */
    private function get_files_handler() {
        $handler_class = \DataMachine\Core\Constants::get_input_handler_class('files');
        
        if (!$handler_class || !class_exists($handler_class)) {
            return new \WP_Error('missing_handler', 'Files handler class not found');
        }
        
        // Pure filter-based architecture: direct handler instantiation with parameter-less constructor
	...
	public function get_module_data_ajax_handler() {
		...
		// Security Note: User data (module_name, prompts) is returned as-is for form field population
		// Frontend JavaScript properly escapes this data using escapeHtml() before DOM insertion
    ...
    public function ajax_get_handler_template() {
        ...
        // Always proceed with data preparation for remote handlers and form output
        {
            ...
            // Security Note: $template_html contains properly escaped content from FormRenderer::render_form_fields()
            // which uses WordPress escaping functions (esc_attr, esc_html, etc.) for all user data
    ...
    public function ajax_get_project_modules() { // Renamed method to fit convention
        ...
        try {
            ...
            // Security Note: module_name is returned as-is for frontend JavaScript processing
            // Frontend code must use proper escaping when displaying this data
