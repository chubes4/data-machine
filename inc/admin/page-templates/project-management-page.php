        ...
        <?php if ( ! empty( $projects ) && is_array( $projects ) ) : ?>
            <?php foreach ( $projects as $project ) : ?>
                ...
                <div class="dm-project-card" 
                    ...
                    <!-- Step Flow Configuration (Mirrored Pipeline) -->
                    <div class="dm-step-flow-configuration" style="margin-bottom: 20px; padding-top: 15px; border-top: 1px solid #eee;">
                        <h4 style="margin: 0 0 12px 0; font-size: 14px; font-weight: 600; color: #23282d; display: flex; align-items: center; gap: 8px;">
                            <span class="dashicons dashicons-networking" style="font-size: 16px; color: #a7aaad;"></span>
                            Data Flow Configuration
                            <span class="dm-flow-steps-count badge" style="background: #f0f0f1; color: #50575e; padding: 2px 8px; border-radius: 12px; font-size: 11px; font-weight: 500; margin-left: auto;">
                                <?php echo esc_html(count($modules ?? [])); ?> modules
                            </span>
                        </h4>
                        
                        <!-- Step Flow Builder -->
                        <div class="dm-step-flow-builder" 
                             data-project-id="<?php echo esc_attr($project->project_id); ?>"
                             style="background: #f8f9fa; border: 1px solid #e2e4e7; border-radius: 6px; padding: 15px;">
                            
                            <!-- Dynamic Step Container -->
                            <div class="dm-dynamic-steps-container" style="min-height: 80px; margin-bottom: 20px;">
                                <!-- Dynamic step cards will be rendered here by JavaScript -->
                                <div class="dm-steps-loading" style="text-align: center; padding: 20px; color: #666; font-size: 12px; font-style: italic;">
                                    No steps configured - click "Add Step" to begin
                                </div>
                            </div>
                            
                            <!-- Add Step Controls -->
                            <div class="dm-add-step-controls" style="display: flex; gap: 8px; align-items: center; margin-bottom: 15px;">
                                <button type="button" class="dm-add-step-btn button" 
                                        data-project-id="<?php echo esc_attr($project->project_id); ?>"
                                        style="font-size: 12px; padding: 6px 12px; height: auto;">
                                    + Add Step
                                </button>
                                <span style="font-size: 11px; color: #666; font-style: italic;">
                                    Build your pipeline by adding and configuring steps
                                </span>
                            </div>
                            
                            <!-- Flow Help Text -->
                            <div class="dm-flow-help" style="padding: 8px 12px; background: #fff; border: 1px solid #ddd; border-radius: 4px; font-size: 11px; color: #666;">
                                <strong>Pipeline Flow:</strong> Steps process data sequentially. 
                                Configure each step to define how data flows through your pipeline. 
                                Use drag-and-drop to reorder steps as needed.
                            </div>
                        </div>
                    </div>
