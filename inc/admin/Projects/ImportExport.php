...
class ImportExport {
	...
	public function handle_project_export() {
		...
		// Ensure clean output buffer for file download
		if ( ob_get_level() ) {
			ob_end_clean();
		}
		
		// Output JSON data securely for file download
		// Note: wp_send_json() cannot be used here as it would interfere with download headers
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JSON data validated and intended for file download.
		echo $json_data;
		exit;
	}
