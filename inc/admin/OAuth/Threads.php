...
class Threads {
    ...
    public function handle_callback(int $user_id, string $code, string $state): bool|WP_Error {
        $this->get_logger()?->info('Handling Threads OAuth callback.', ['user_id' => $user_id]);
        ...
        if (empty($stored_state) || !hash_equals($stored_state, $state)) {
            $this->get_logger()?->error('Threads OAuth Error: State mismatch.', ['user_id' => $user_id]);
        ...
        if (is_wp_error($response)) {
            $this->get_logger()?->error('Threads OAuth Error: Token request failed.', ['user_id' => $user_id, 'error' => $response->get_error_message()]);
        ...
        if ($http_code !== 200 || empty($data['access_token'])) {
            ...
            $this->get_logger()?->error('Threads OAuth Error: Token exchange failed.', ['user_id' => $user_id, 'http_code' => $http_code, 'response' => $body]);
        ...
        $this->get_logger()?->info('Threads OAuth: Exchanging short-lived token for long-lived token.', ['user_id' => $user_id]);
        $exchange_params = [
        ...
        if (is_wp_error($exchange_response)) {
            $this->get_logger()?->error('Threads OAuth Error: Long-lived token exchange request failed (HTTP).', ['user_id' => $user_id, 'error' => $exchange_response->get_error_message()]);
        ...
        if ($exchange_http_code !== 200 || empty($exchange_data['access_token'])) {
            ...
            $this->get_logger()?->error('Threads OAuth Error: Long-lived token exchange failed (API).', ['user_id' => $user_id, 'http_code' => $exchange_http_code, 'response' => $exchange_body]);
        ...
        $this->get_logger()?->info('Threads OAuth: Successfully exchanged for long-lived token.', ['user_id' => $user_id]);
        ...
        if (!is_wp_error($posting_entity_info) && isset($posting_entity_info['id'])) {
            ...
            $this->get_logger()?->info('Fetched posting entity info from /me.', ['user_id' => $user_id, 'posting_entity_id' => $posting_entity_info['id'], 'posting_entity_name' => $account_details['page_name']]);
        } else {
            // Critical error if /me doesn't return the necessary ID
            $this->get_logger()?->error('Threads OAuth Error: Failed to fetch posting entity info from /me endpoint.', [
                'user_id' => $user_id,
        ...
        if ($account_details['access_token'] === false) {
             $this->get_logger()?->error('Threads OAuth Error: Failed to encrypt access token.', ['user_id' => $user_id]);
        ...
        $this->get_logger()?->info('Threads account authenticated and token stored.', ['user_id' => $user_id, 'page_id' => $account_details['page_id']]);
    ...
    private function get_facebook_user_profile(string $access_token): array|WP_Error {
        ...
        $this->get_logger()?->debug('Facebook Graph API: Fetching authenticating user profile.', ['url' => $url]);
        ...
        if (is_wp_error($response)) {
             $this->get_logger()?->error('Facebook Graph API Error: Profile fetch wp_remote_get failed.', ['error' => $response->get_error_message()]);
            return $response;
        }
        ...
        if ($http_code !== 200 || isset($data['error'])) {
             ...
             $this->get_logger()?->error('Facebook Graph API Error: Profile fetch failed.', ['http_code' => $http_code, 'response' => $body]);
             return new WP_Error('fb_profile_fetch_failed', $error_message, $data);
        }

         if (empty($data['id'])) {
             $this->get_logger()?->error('Facebook Graph API Error: Profile fetch response missing ID.', ['http_code' => $http_code, 'response' => $body]);
             return new WP_Error('fb_profile_id_missing', __('Profile ID missing in response from Facebook Graph API.', 'data-machine'), $data);
         }

        $this->get_logger()?->debug('Facebook Graph API: Profile fetched successfully.', ['profile_id' => $data['id']]);
    ...
    public function handle_admin_page_oauth_callback() {
        ...
        if (isset($_GET['error'])) {
            ...
            $this->get_logger()?->error('Threads OAuth Error (Callback Init): User denied access or error occurred.', ['error' => $error, 'description' => $error_description]);
        ...
        if (isset($_GET['code']) && isset($_GET['state'])) {
            ...
            if (!$stored_state || !hash_equals($stored_state, $received_state)) {
                $this->get_logger()?->error('Threads OAuth Error (Callback): State mismatch.', ['user_id' => $user_id, 'received' => $received_state, 'stored' => $stored_state]);
