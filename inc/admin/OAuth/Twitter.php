...
class Twitter {
    ...
    public function handle_oauth_init() {
        ...
        try {
            ...
            if ($connection->getLastHttpCode() != 200 || !isset($request_token['oauth_token']) || !isset($request_token['oauth_token_secret'])) {
                ...
                $this->get_logger()?->error('Twitter OAuth Error: ' . $error_message, [
            ...
            $this->get_logger()?->error('Twitter OAuth Exception during init: ' . $e->getMessage());
    ...
    public function handle_oauth_callback() {
        ...
        if (isset($_GET['denied'])) {
            ...
            $this->get_logger()?->warning('Twitter OAuth Warning: User denied access.', ['user_id' => $user_id, 'denied_token' => $denied_token]);
        ...
        if (!isset($_GET['oauth_token']) || !isset($_GET['oauth_verifier'])) {
            $this->get_logger()?->error('Twitter OAuth Error: Missing oauth_token or oauth_verifier in callback.', ['user_id' => $user_id, 'query_params' => $_GET]);
        ...
        if (empty($oauth_token_secret)) {
            $this->get_logger()?->error('Twitter OAuth Error: Request token secret missing or expired in transient.', ['user_id' => $user_id, 'oauth_token' => $oauth_token]);
        ...
        $apiSecret = get_option('twitter_api_secret');
        if (empty($apiKey) || empty($apiSecret)) {
            $this->get_logger()?->error('Twitter OAuth Error: API Key/Secret missing during callback.', ['user_id' => $user_id]);
            wp_redirect(admin_url('admin.php?page=dm-api-keys&auth_error=twitter_missing_app_keys'));
            exit;
        }
        ...
        try {
            ...
            if ($connection->getLastHttpCode() != 200 || !isset($access_token_data['oauth_token']) || !isset($access_token_data['oauth_token_secret'])) {
                $this->get_logger()?->error('Twitter OAuth Error: Failed to get access token.', [
                    'user_id' => $user_id,
                    'http_code' => $connection->getLastHttpCode(),
            ...
            if ($encrypted_access_token === false || $encrypted_access_token_secret === false) {
                $this->get_logger()?->error('Twitter OAuth Error: Failed to encrypt access tokens.', ['user_id' => $user_id]);
            ...
            $this->get_logger()?->error('Twitter OAuth Exception during callback: ' . $e->getMessage(), ['user_id' => $user_id]);
    ...
    public function get_connection_for_user(int $user_id): TwitterOAuth|WP_Error {
        $this->get_logger()?->info('Attempting to get authenticated Twitter connection for user.', ['user_id' => $user_id]);
        ...
        if (empty($credentials) || empty($credentials['access_token']) || empty($credentials['access_token_secret'])) {
            $this->get_logger()?->error('Missing Twitter credentials in user meta.', ['user_id' => $user_id]);
        ...
        if ($decrypted_access_token === false || $decrypted_access_token_secret === false) {
            $this->get_logger()?->error('Failed to decrypt Twitter credentials.', ['user_id' => $user_id]);
        ...
        if (empty($consumer_key) || empty($consumer_secret)) {
            $this->get_logger()?->error('Missing Twitter API key/secret in site options.', ['user_id' => $user_id]);
        ...
        try {
            ...
            $this->get_logger()?->info('Successfully created authenticated Twitter connection for user.', ['user_id' => $user_id]);
            return $connection;
        } catch (\Exception $e) {
            $this->get_logger()?->error('Exception creating TwitterOAuth connection: ' . $e->getMessage(), ['user_id' => $user_id]);
