...
class Data_Machine_Job_Status_Manager {
	...
	/**
	 * Database projects service for updating last_run_at.
	 ...
	 * @param Data_Machine_Database_Projects|null $db_projects Projects database service.
	 * @param Data_Machine_Logger|null $logger Logger instance.
	 */
	public function __construct(
		Data_Machine_Database_Jobs $db_jobs,
		?Data_Machine_Database_Projects $db_projects = null,
	...
	public function complete(int $job_id, string $status = 'completed', ?array $result_data = null, ?string $message = null): bool {
		...
		if (!in_array($status, ['completed', 'completed_with_errors'], true)) {
			$this->logger?->error("Invalid completion status: {$status}", ['job_id' => $job_id]);
			return false;
		}
		...
		if ($result_data !== null) {
			...
			if ($json_data === false) {
				$this->logger?->error('Failed to encode result data for job completion', ['job_id' => $job_id]);
		...
		$success = $this->db_jobs->complete_job($job_id, $status, $json_data);
		
		if ($success) {
			$this->logger?->info($completion_message, [
				'job_id' => $job_id,
				'final_status' => $status,
	...
	public function fail(int $job_id, string $error_message): bool {
		...
		$success = $this->db_jobs->complete_job($job_id, 'failed', $error_data);
		
		if ($success) {
			$this->logger?->error("Job failed: {$error_message}", [
				'job_id' => $job_id,
				'final_status' => 'failed'
			]);
		} else {
			$this->logger?->error("Failed to update job status to failed", [
				'job_id' => $job_id,
	...
	private function transition_status(int $job_id, string $new_status, string $log_message): bool {
		...
		if ($current_status === null) {
			$this->logger?->error("Job not found for status transition", ['job_id' => $job_id]);
			return false;
		}
		...
		if ($success) {
			...
			$this->logger?->error("Failed to transition job status", [
				'job_id' => $job_id,
				'current_status' => $current_status,
