 ...
 * @since      0.15.0
 */
class Data_Machine_Base_Output_Handler {
     ...
     * @param string $handler_context Handler name for logging context
     * @return Data_Machine_AI_Response_Parser|WP_Error Parser instance or error
     */
    protected function load_ai_response_parser($ai_output_string, $handler_context = 'Output Handler') {
        ...
        // Load the parser class if not already available
        if (!class_exists('Data_Machine_AI_Response_Parser')) {
            if (!file_exists($parser_path)) {
                $this->logger && $this->logger->error($handler_context . ': AI Response Parser class file not found.', [
                    'expected_path' => $parser_path
                ]);
        ...
        // Double-check class loaded successfully
        if (!class_exists('Data_Machine_AI_Response_Parser')) {
            $this->logger && $this->logger->error($handler_context . ': AI Response Parser class failed to load.');
            return new WP_Error('parser_load_failed', __('Failed to load AI Response Parser.', 'data-machine'));
        }
