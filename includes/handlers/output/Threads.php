...
class Threads extends BaseOutputHandler {
	...
	public function handle( string $ai_output_string, array $module_job_config, ?int $user_id, array $input_metadata ): array|WP_Error {
		...
		$include_hashtags = !empty($output_config['include_hashtags']);
		$post_type = $output_config['post_type'] ?? 'auto';
		...
		$access_token = \DataMachine\Admin\OAuth\Threads::get_access_token($user_id);
		...
		// Append hashtags to content if enabled and available
		if ($include_hashtags && !empty($hashtags)) {
		...
		// Note: CAROUSEL posts require multiple container creations - not implemented yet
		...
		if ($container_id) {
			...
			try {
				...
				if ($http_code >= 200 && $http_code < 300 && isset($data['id'])) {
					...
					// Container cleanup could be implemented here if needed
	...
	public static function get_settings_fields(array $current_config = []): array {
		...
		return [
			'include_images' => [
				...
				'description' => __('Attempt to post images from the original content when available.', 'data-machine'),
				'default' => false
			],
			'include_hashtags' => [
				'type' => 'checkbox',
				'label' => __('Include Hashtags', 'data-machine'),
				'description' => __('Append hashtags extracted from AI output to the post.', 'data-machine'),
				'default' => true
			],
			'post_type' => [
				'type' => 'select',
				'label' => __('Post Type Preference', 'data-machine'),
				'description' => __('Preferred post type when content allows multiple options.', 'data-machine'),
				'options' => [
					'auto' => __('Auto-detect', 'data-machine'),
					'text' => __('Text only', 'data-machine'),
					'image' => __('Image preferred', 'data-machine')
				],
				'default' => 'auto'
			]
		];
	...
	public function sanitize_settings(array $raw_settings): array {
		...
		$sanitized['include_hashtags'] = !empty($raw_settings['include_hashtags']);
		$sanitized['post_type'] = in_array($raw_settings['post_type'] ?? 'auto', ['auto', 'text', 'image']) 
			? $raw_settings['post_type'] : 'auto';
		return $sanitized;
	}
}